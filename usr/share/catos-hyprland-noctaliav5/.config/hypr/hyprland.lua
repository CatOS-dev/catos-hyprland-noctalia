...
-- Monitors are configured in the profile-scoped seed
-- custom/catos-hyprland-noctaliav5/monitors.lua.
...
-- Input, gesture and device settings live in the profile-scoped seed
-- custom/catos-hyprland-noctaliav5/input.lua.
...
-- Profile-scoped user customizations. These files are seeds, not managed paths.
-- Load them last so user settings override the managed defaults above.
local function load_custom(module)
    local ok, err = pcall(require, module)
    if not ok and not string.find(err, "not found", 1, true) then
        error("custom " .. module .. ": " .. tostring(err), 0)
    end
end

load_custom("custom.catos-hyprland-noctaliav5.input")
load_custom("custom.catos-hyprland-noctaliav5.monitors")
load_custom("custom.catos-hyprland-noctaliav5.rules")
